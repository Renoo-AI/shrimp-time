      ...
      >
        ...
        <div className="flex items-center gap-2">
          <motion.button
            ...
            className={`rounded-[30px] font-black tracking-widest uppercase cursor-pointer text-brand-navy premium-gold-gradient shadow-[0_4px_15px_rgba(234,209,27,0.3)] hover:shadow-[0_4px_20px_rgba(234,209,27,0.5)] transition-all flex items-center gap-2 ${
              isScrolled 
                ? 'px-4 py-2 text-[9px]' 
                : 'px-5 py-2.5 text-[10px]'
            }`}
          >
            <Calendar size={isScrolled ? 11 : 12} />
            <span>Réserver</span>
          </motion.button>
          ...
          {isScrolled ? (
            <button
              ...
              className="p-1.5 text-white hover:text-brand-yellow focus:outline-none cursor-pointer transition-colors"
            ...
            <div className="md:hidden flex items-center ml-1">
              <button
                onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                className="p-1 text-white hover:text-brand-yellow focus:outline-none cursor-pointer transition-colors"
                aria-label="Toggle Navigation Menu"
              >
